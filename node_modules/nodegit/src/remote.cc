...
GitRemote::GitRemote(git_remote *raw, bool selfFreeing, bool shouldDuplicate)
{
  if (shouldDuplicate) {
    (&this->raw, raw);
    selfFreeing = true;
...
Local<v8::Value> GitRemote::New(const git_remote *raw, bool selfFreeing, bool shouldDuplicate)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), Nan::New(shouldDuplicate) };
  return scope.Escape(Nan::NewInstance(Nan::New(GitRemote::constructor_template), 3, argv).ToLocalChecked());
}
...
// start convert_to_v8 block
    ...
    if (baton->out != NULL) {
      // GitRemote baton->out
      to = GitRemote::New(baton->out, false  );
    } else {
      to = Nan::Null();
    }
...
// start convert_to_v8 block
    ...
    if (baton->dest != NULL) {
      ...
      to = GitRemote::New(baton->dest, false  );
    } else {
      to = Nan::Null();
    }
...
// start convert_to_v8 block
    ...
    if (result != NULL) {
      // GitRefspec result
      to = GitRefspec::New(result, false  );
    } else {
      to = Nan::Null();
    }
