...
GitConfig::GitConfig(git_config *raw, bool selfFreeing, bool shouldDuplicate)
{
  if (shouldDuplicate) {
    (&this->raw, raw);
    selfFreeing = true;
...
Local<v8::Value> GitConfig::New(const git_config *raw, bool selfFreeing, bool shouldDuplicate)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), Nan::New(shouldDuplicate) };
  return scope.Escape(Nan::NewInstance(Nan::New(GitConfig::constructor_template), 3, argv).ToLocalChecked());
}
...
// end convert_from_v8 block
  ...
  {
    LockMaster lockMaster(true        ,    Nan::ObjectWrap::Unwrap<GitConfig>(info.This())->GetValue()
                          ,    from_name
                         );
...
void GitConfig::SetStringWorker::Execute()
{
  ...
  {
    LockMaster lockMaster(true        ,baton->cfg
                          ,baton->name
                          ,baton->value
                         );
...
void GitConfig::SnapshotWorker::Execute()
{
  ...
  {
    LockMaster lockMaster(true        ,baton->config
                         );
