...
GitStatusList::GitStatusList(git_status_list *raw, bool selfFreeing, bool shouldDuplicate)
{
  if (shouldDuplicate) {
    (&this->raw, raw);
    selfFreeing = true;
...
Local<v8::Value> GitStatusList::New(const git_status_list *raw, bool selfFreeing, bool shouldDuplicate)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), Nan::New(shouldDuplicate) };
  return scope.Escape(Nan::NewInstance(Nan::New(GitStatusList::constructor_template), 3, argv).ToLocalChecked());
}
...
NAN_METHOD(GitStatusList::Entrycount)
{
  ...
  {
    LockMaster lockMaster(true        ,    Nan::ObjectWrap::Unwrap<GitStatusList>(info.This())->GetValue()
                         );
