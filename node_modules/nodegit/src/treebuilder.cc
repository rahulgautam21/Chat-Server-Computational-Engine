...
GitTreebuilder::GitTreebuilder(git_treebuilder *raw, bool selfFreeing, bool shouldDuplicate)
{
  if (shouldDuplicate) {
    (&this->raw, raw);
    selfFreeing = true;
...
Local<v8::Value> GitTreebuilder::New(const git_treebuilder *raw, bool selfFreeing, bool shouldDuplicate)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), Nan::New(shouldDuplicate) };
  return scope.Escape(Nan::NewInstance(Nan::New(GitTreebuilder::constructor_template), 3, argv).ToLocalChecked());
}
...
NAN_METHOD(GitTreebuilder::Clear)
{
  ...
  {
    LockMaster lockMaster(true        ,    Nan::ObjectWrap::Unwrap<GitTreebuilder>(info.This())->GetValue()
                         );
...
void GitTreebuilder::InsertWorker::Execute()
{
  ...
  {
    LockMaster lockMaster(true        ,baton->bld
                          ,baton->filename
                          ,baton->id
                         );
