...
GitDiffBinary::GitDiffBinary(git_diff_binary *raw, bool selfFreeing, bool shouldDuplicate)
{
  if (shouldDuplicate) {
    (&this->raw, raw);
    selfFreeing = true;
...
Local<v8::Value> GitDiffBinary::New(const git_diff_binary *raw, bool selfFreeing, bool shouldDuplicate)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), Nan::New(shouldDuplicate) };
  return scope.Escape(Nan::NewInstance(Nan::New(GitDiffBinary::constructor_template), 3, argv).ToLocalChecked());
}
