...
GitAnnotatedCommit::GitAnnotatedCommit(git_annotated_commit *raw, bool selfFreeing, bool shouldDuplicate)
{
  if (shouldDuplicate) {
    (&this->raw, raw);
    selfFreeing = true;
...
Local<v8::Value> GitAnnotatedCommit::New(const git_annotated_commit *raw, bool selfFreeing, bool shouldDuplicate)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), Nan::New(shouldDuplicate) };
  return scope.Escape(Nan::NewInstance(Nan::New(GitAnnotatedCommit::constructor_template), 3, argv).ToLocalChecked());
}
...
NAN_METHOD(GitAnnotatedCommit::Free)
{
  ...
  if (Nan::ObjectWrap::Unwrap<GitAnnotatedCommit>(info.This())->GetValue() != NULL) {
    ...
    {
      LockMaster lockMaster(true        ,    Nan::ObjectWrap::Unwrap<GitAnnotatedCommit>(info.This())->GetValue()
                           );
