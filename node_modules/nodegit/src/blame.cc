...
GitBlame::GitBlame(git_blame *raw, bool selfFreeing, bool shouldDuplicate)
{
  if (shouldDuplicate) {
    (&this->raw, raw);
    selfFreeing = true;
...
Local<v8::Value> GitBlame::New(const git_blame *raw, bool selfFreeing, bool shouldDuplicate)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), Nan::New(shouldDuplicate) };
  return scope.Escape(Nan::NewInstance(Nan::New(GitBlame::constructor_template), 3, argv).ToLocalChecked());
}
...
void GitBlame::BufferWorker::Execute()
{
  ...
  {
    LockMaster lockMaster(true        ,baton->reference
                          ,baton->buffer
                         );
...
NAN_METHOD(GitBlame::Free)
{
  ...
  if (Nan::ObjectWrap::Unwrap<GitBlame>(info.This())->GetValue() != NULL) {
    ...
    {
      LockMaster lockMaster(true        ,    Nan::ObjectWrap::Unwrap<GitBlame>(info.This())->GetValue()
                           );
