...
GitOdbObject::GitOdbObject(git_odb_object *raw, bool selfFreeing, bool shouldDuplicate)
{
  if (shouldDuplicate) {
    (&this->raw, raw);
    selfFreeing = true;
...
Local<v8::Value> GitOdbObject::New(const git_odb_object *raw, bool selfFreeing, bool shouldDuplicate)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), Nan::New(shouldDuplicate) };
  return scope.Escape(Nan::NewInstance(Nan::New(GitOdbObject::constructor_template), 3, argv).ToLocalChecked());
}
...
NAN_METHOD(GitOdbObject::Data)
{
  ...
  {
    LockMaster lockMaster(true        ,    Nan::ObjectWrap::Unwrap<GitOdbObject>(info.This())->GetValue()
                         );
