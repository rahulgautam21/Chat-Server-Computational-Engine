...
GitPackbuilder::GitPackbuilder(git_packbuilder *raw, bool selfFreeing, bool shouldDuplicate)
{
  if (shouldDuplicate) {
    (&this->raw, raw);
    selfFreeing = true;
...
Local<v8::Value> GitPackbuilder::New(const git_packbuilder *raw, bool selfFreeing, bool shouldDuplicate)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), Nan::New(shouldDuplicate) };
  return scope.Escape(Nan::NewInstance(Nan::New(GitPackbuilder::constructor_template), 3, argv).ToLocalChecked());
}
...
NAN_METHOD(GitPackbuilder::Free)
{
  ...
  if (Nan::ObjectWrap::Unwrap<GitPackbuilder>(info.This())->GetValue() != NULL) {
    ...
    {
      LockMaster lockMaster(true        ,    Nan::ObjectWrap::Unwrap<GitPackbuilder>(info.This())->GetValue()
                           );
