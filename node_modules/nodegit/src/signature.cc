...
GitSignature::GitSignature(git_signature *raw, bool selfFreeing, bool shouldDuplicate)
{
  if (shouldDuplicate) {
...
Local<v8::Value> GitSignature::New(const git_signature *raw, bool selfFreeing, bool shouldDuplicate)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), Nan::New(shouldDuplicate) };
  return scope.Escape(Nan::NewInstance(Nan::New(GitSignature::constructor_template), 3, argv).ToLocalChecked());
}
...
void GitSignature::DupWorker::Execute()
{
  ...
  {
    LockMaster lockMaster(true        ,baton->sig
                         );
...
NAN_METHOD(GitSignature::Free)
{
  ...
  if (Nan::ObjectWrap::Unwrap<GitSignature>(info.This())->GetValue() != NULL) {
    ...
    {
      LockMaster lockMaster(true        ,    Nan::ObjectWrap::Unwrap<GitSignature>(info.This())->GetValue()
                           );
...
// end convert_from_v8 block
  ...
  {
    LockMaster lockMaster(true        ,    from_name
                          ,    from_email
                         );
