...
GitRepository::GitRepository(git_repository *raw, bool selfFreeing, bool shouldDuplicate)
{
  if (shouldDuplicate) {
    (&this->raw, raw);
    selfFreeing = true;
...
Local<v8::Value> GitRepository::New(const git_repository *raw, bool selfFreeing, bool shouldDuplicate)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), Nan::New(shouldDuplicate) };
  return scope.Escape(Nan::NewInstance(Nan::New(GitRepository::constructor_template), 3, argv).ToLocalChecked());
}
...
NAN_METHOD(GitRepository::DetachHead)
{
  ...
  {
    LockMaster lockMaster(true        ,    Nan::ObjectWrap::Unwrap<GitRepository>(info.This())->GetValue()
                         );
...
// start convert_to_v8 block
    ...
    if (baton->out != NULL) {
      // GitRefs baton->out
      to = GitRefs::New(baton->out, false  );
    } else {
      to = Nan::Null();
    }
...
void GitRepository::InitWorker::Execute()
{
  ...
  {
    LockMaster lockMaster(true        ,baton->path
                         );
...
void GitRepository::InitExtWorker::Execute()
{
  ...
  {
    LockMaster lockMaster(true        ,baton->repo_path
                          ,baton->opts
                         );
...
void GitRepository::OpenBareWorker::Execute()
{
  ...
  {
    LockMaster lockMaster(true        ,baton->bare_path
                         );
...
void GitRepository::WrapOdbWorker::Execute()
{
  ...
  {
    LockMaster lockMaster(true        ,baton->odb
                         );
