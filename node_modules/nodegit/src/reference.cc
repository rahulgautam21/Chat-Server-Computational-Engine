...
GitRefs::GitRefs(git_reference *raw, bool selfFreeing, bool shouldDuplicate)
{
  if (shouldDuplicate) {
    (&this->raw, raw);
    selfFreeing = true;
...
Local<v8::Value> GitRefs::New(const git_reference *raw, bool selfFreeing, bool shouldDuplicate)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), Nan::New(shouldDuplicate) };
  return scope.Escape(Nan::NewInstance(Nan::New(GitRefs::constructor_template), 3, argv).ToLocalChecked());
}
...
// end convert_from_v8 block
  ...
  {
    LockMaster lockMaster(true        ,    Nan::ObjectWrap::Unwrap<GitRefs>(info.This())->GetValue()
                          ,    from_ref2
                         );
...
// end convert_from_v8 block
  ...
  {
    LockMaster lockMaster(true        ,    from_refname
                         );
...
// end convert_from_v8 block
  ...
  {
    LockMaster lockMaster(true        ,    from_buffer_out
                          ,    from_name
                         );
...
void GitRefs::PeelWorker::Execute()
{
  ...
  {
    LockMaster lockMaster(true        ,baton->ref
                         );
...
// start convert_to_v8 block
    ...
    if (baton->new_ref != NULL) {
      ...
      to = GitRefs::New(baton->new_ref, false  );
    } else {
      to = Nan::Null();
    }
