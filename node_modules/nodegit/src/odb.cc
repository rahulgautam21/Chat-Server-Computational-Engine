...
GitOdb::GitOdb(git_odb *raw, bool selfFreeing, bool shouldDuplicate)
{
  if (shouldDuplicate) {
    (&this->raw, raw);
    selfFreeing = true;
...
Local<v8::Value> GitOdb::New(const git_odb *raw, bool selfFreeing, bool shouldDuplicate)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), Nan::New(shouldDuplicate) };
  return scope.Escape(Nan::NewInstance(Nan::New(GitOdb::constructor_template), 3, argv).ToLocalChecked());
}
...
// end convert_from_v8 block
  ...
  {
    LockMaster lockMaster(true        ,    Nan::ObjectWrap::Unwrap<GitOdb>(info.This())->GetValue()
                          ,    from_path
                         );
...
void GitOdb::OpenWorker::Execute()
{
  ...
  {
    LockMaster lockMaster(true        ,baton->objects_dir
                         );
...
void GitOdb::ReadWorker::Execute()
{
  ...
  {
    LockMaster lockMaster(true        ,baton->db
                          ,baton->id
                         );
