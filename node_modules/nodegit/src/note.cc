...
GitNote::GitNote(git_note *raw, bool selfFreeing, bool shouldDuplicate)
{
  if (shouldDuplicate) {
    (&this->raw, raw);
    selfFreeing = true;
...
Local<v8::Value> GitNote::New(const git_note *raw, bool selfFreeing, bool shouldDuplicate)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), Nan::New(shouldDuplicate) };
  return scope.Escape(Nan::NewInstance(Nan::New(GitNote::constructor_template), 3, argv).ToLocalChecked());
}
...
NAN_METHOD(GitNote::Author)
{
  ...
  {
    LockMaster lockMaster(true        ,    Nan::ObjectWrap::Unwrap<GitNote>(info.This())->GetValue()
                         );
...
// end convert_from_v8 block
  ...
  {
    LockMaster lockMaster(true        ,    from_note_id
                          ,    from_annotated_id
                          ,    from_it
                         );
