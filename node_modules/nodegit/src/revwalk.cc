...
GitRevwalk::GitRevwalk(git_revwalk *raw, bool selfFreeing, bool shouldDuplicate)
{
  if (shouldDuplicate) {
    (&this->raw, raw);
    selfFreeing = true;
...
Local<v8::Value> GitRevwalk::New(const git_revwalk *raw, bool selfFreeing, bool shouldDuplicate)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), Nan::New(shouldDuplicate) };
  return scope.Escape(Nan::NewInstance(Nan::New(GitRevwalk::constructor_template), 3, argv).ToLocalChecked());
}
...
// end convert_from_v8 block
  ...
  {
    LockMaster lockMaster(true        ,    Nan::ObjectWrap::Unwrap<GitRevwalk>(info.This())->GetValue()
                          ,    from_commit_id
                         );
