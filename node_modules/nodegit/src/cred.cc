...
GitCred::GitCred(git_cred *raw, bool selfFreeing, bool shouldDuplicate)
{
  if (shouldDuplicate) {
    (&this->raw, raw);
    selfFreeing = true;
...
Local<v8::Value> GitCred::New(const git_cred *raw, bool selfFreeing, bool shouldDuplicate)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), Nan::New(shouldDuplicate) };
  return scope.Escape(Nan::NewInstance(Nan::New(GitCred::constructor_template), 3, argv).ToLocalChecked());
}
...
NAN_METHOD(GitCred::HasUsername)
{
  ...
  {
    LockMaster lockMaster(true        ,    Nan::ObjectWrap::Unwrap<GitCred>(info.This())->GetValue()
                         );
...
// end convert_from_v8 block
  ...
  {
    LockMaster lockMaster(true        ,    from_username
                         );
...
void GitCred::SshKeyMemoryNewWorker::Execute()
{
  ...
  {
    LockMaster lockMaster(true        ,baton->username
                          ,baton->publickey
                          ,baton->privatekey
                          ,baton->passphrase
                         );
