...
GitRebase::GitRebase(git_rebase *raw, bool selfFreeing, bool shouldDuplicate)
{
  if (shouldDuplicate) {
    (&this->raw, raw);
    selfFreeing = true;
...
Local<v8::Value> GitRebase::New(const git_rebase *raw, bool selfFreeing, bool shouldDuplicate)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), Nan::New(shouldDuplicate) };
  return scope.Escape(Nan::NewInstance(Nan::New(GitRebase::constructor_template), 3, argv).ToLocalChecked());
}
...
NAN_METHOD(GitRebase::Abort)
{
  ...
  {
    LockMaster lockMaster(true        ,    Nan::ObjectWrap::Unwrap<GitRebase>(info.This())->GetValue()
                         );
...
void GitRebase::NextWorker::Execute()
{
  ...
  {
    LockMaster lockMaster(true        ,baton->rebase
                         );
